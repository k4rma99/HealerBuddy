...
export default function Hero() {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            ...
            // Set the opacity to 0, triggering the transition
            setImageOpacity(0);

            // After a short delay, set the opacity back to 1
            setTimeout(() => {
                setImageOpacity(0.5);
            }, 100);
        }, 3000);
            ...
            <div className="carousel relative w-full" ref={carouselRef}>
                {
                    <div id={"slide" + keyIndex as string} className="carousel-item relative w-full">
                        <Image
                            className="z-10 h-full w-full transition-opacity duration-500"
